...
traits
gwas <- floret_site_utilization_gwas
...
#### THIS IS FOR FLAG LEAF AREA WHERE THERE ARE TONS OF PRIORITY SNPS ON CHR 9
...
traits
phenotype <- floret_site_utilization
gwas <- floret_site_utilization_gwas
    ...
    if (nrow(anova_snps_nest$data[[j]]) > 1) { # if there's more than one on a chromosome
      ...
      for (k in 1:nrow(keep)) {
        if (keep$SNP[k] %in% priority_snps$SNP) { # if a SNP is in the priority SNP list
          ...
          test <- keep %>% mutate(bp_pos = as.numeric(substr(SNP, 7, nchar(SNP))), # calculate the base pair difference between the remaining SNPs
...
for (i in 1:length(gwas)) {
  gwas[[i]] <- gwas[[i]] %>% mutate(trait = "flag_leaf_area",
                                    loc = loc[i], 
                                    year = year[i])
}
gwas[[4]]$loc <- "TLI"
...
write.csv(gwas_final, paste(dir, folder, "/output/final_gwas_flag_leaf_area.csv", sep = ""), row.names = FALSE)
